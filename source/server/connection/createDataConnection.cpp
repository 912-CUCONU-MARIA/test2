...
//    return dataConnectionfd;
    ...
    printInfo("[SERVER] : Creating a new data Connection");

    const char* ip = getDataConnectionIP();
    int port = getDataConnectionPortNumber();

    // Attempt to connect to the client's IP and port
    int dataConnectionfd = createSocketAndConnectToHost(ip, port);
    if (dataConnectionfd < 0) {
        // Handle error: unable to establish data connection
        logs("Failed to establish data connection with client.");
        // Send a response to the client indicating failure to establish a data connection
        Send(controlConnectionfd, "425 Can't open data connection.", 425);
        return -1;
    }
